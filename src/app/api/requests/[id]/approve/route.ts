...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  demoRequests[idx] = { ...demoRequests[idx], status: "APPROVED", updatedAt: new Date().toISOString() };

  const body = await req.json().catch(() => ({}));
  const actor = demoUsers.find((u) => u.id === session.userId);
  demoCaseHistory.push({ id: getNextHistoryId(), requestId: demoRequests[idx].id, userId: session.userId, action: "APPROVED", comment: body.comment || null, createdAt: new Date().toISOString(), user: { name: actor?.name || "" } });
