...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  demoRequests[idx] = { ...demoRequests[idx], status: "CANCELLED", cancelReason: body.reason, updatedAt: new Date().toISOString() };

  const actor = demoUsers.find((u) => u.id === session.userId);
  demoCaseHistory.push({ id: getNextHistoryId(), requestId: demoRequests[idx].id, userId: session.userId, action: "CANCELLED", comment: body.reason, createdAt: new Date().toISOString(), user: { name: actor?.name || "" } });
