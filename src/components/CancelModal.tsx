  ...
  return (
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl p-6 w-full max-w-sm">
        <h3 className="text-lg font-bold mb-4">{t("modal.cancelTitle")}</h3>
        <div className="space-y-3">
          ...
          <div className="flex gap-2 justify-end pt-2">
            ...
            <button onClick={() => { if (reason) { onCancel(reason); onClose(); } }}
              className="px-4 py-2 bg-orange-600 text-white rounded-lg text-sm font-semibold hover:bg-orange-700">{t("modal.confirmCancel")}</button>
          </div>
        </div>
      </div>
    </div>
