  ...
  return (
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl p-6 w-full max-w-sm">
        <h3 className="text-lg font-bold mb-4">{t("modal.assignTitle")}</h3>
        <div className="space-y-3">
          <div>
            <label className="block text-sm font-semibold mb-1">{t("modal.selectAssignee")}</label>
          ...
          <div>
            <label className="block text-sm font-semibold mb-1">{t("modal.note")}</label>
            <textarea value={comment} onChange={(e) => setComment(e.target.value)} rows={2}
              className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm" placeholder={t("modal.notePlaceholder")} />
          </div>
          <div className="flex gap-2 justify-end pt-2">
            ...
            <button onClick={() => { onAssign(assigneeId, comment); onClose(); }}
              className="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-semibold hover:bg-blue-700">{t("action.assign")}</button>
          </div>
        </div>
      </div>
    </div>
